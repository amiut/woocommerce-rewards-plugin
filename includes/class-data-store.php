...
class Data_Store {
	 ...
	 * Ran through `dweb_cr_data_stores`.
	 *
	 * @var array
	 */
	...
	public function __construct( $object_type ) {
		...
		$this->stores      = apply_filters( 'dweb_cr_data_stores', $this->stores );
		...
		if ( array_key_exists( $object_type, $this->stores ) ) {
			$store = apply_filters( 'dweb_cr_' . $object_type . '_data_store', $this->stores[ $object_type ] );
			if ( is_object( $store ) ) {
				if ( ! $store instanceof \Dornaweb\CustomerRewards\Interfaces\Object_Data_Store_Interface ) {
					throw new \Exception( __( 'Invalid data store.', 'dweb_cr' ) );
				}
	 ...
	 *
	 * @since 1.0.0
	 * @param DATA $data dweb_cr data instance.
	 */
